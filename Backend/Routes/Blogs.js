...
blogRoute.post("/create", upload.single("blogImage"), async (req, res) => {
  try {
    ...
    generateSummary(blog).then(summary => {
      newBlog.aiSummary = summary;
      newBlog.aiSummaryGeneratedAt = new Date();
      return newBlog.save();
    })
...
blogRoute.post("/saveblog", async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: "Blog saved successfully", savedBlogs: user.saveBlogs });
  } catch (error) {
    console.log(error);
...
blogRoute.post("/unsavedBlog", async (req, res) => {
  ...
  try {
    ...
    if (result.modifiedCount === 0) {
      return res.status(400).json({ message: "Blog not in saved list" });
    }
