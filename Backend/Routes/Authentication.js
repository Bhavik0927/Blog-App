...
authRouter.post("/signup", upload.single("profilePic"), async (req, res) => {
  try {
    ...
    if (!firstname || !lastname || !email || !password || !role || !req.file) {
      return res
        .status(400)
        .json({ message: "All fields including profilePic are required" });
    }
    ...
    res
      .status(200)
      .send({ message: "User is created", user: userWithoutPassword });
  } catch (error) {
...
authRouter.post("/login", async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: "Login successfully", token: token, login_user });
  } catch (error) {
